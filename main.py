...
async def create_tables():
    try:
        async with engine.begin() as conn:
            # Faqat mavjud bo'lmagan jadvallarni yaratadi, mavjudlarini o'zgartirmaydi
            await conn.run_sync(Base.metadata.create_all)

            # Migration: bot_settings.value ustunini TEXT ga o'zgartirish
            # PostgreSQL
            try:
                await conn.execute(
                    text("ALTER TABLE bot_settings ALTER COLUMN value TYPE TEXT")
                )
                logger.info("✅ bot_settings.value -> TEXT migration bajarildi.")
            except Exception:
                pass  # Allaqachon TEXT yoki SQLite — muammo yo'q

        logger.info("✅ Jadvallar tekshirildi / yaratildi.")
    except Exception as e:
        logger.error(f"❌ Ma'lumotlar bazasiga ulanishda xatolik: {e}")
        logger.error("DATABASE_URL ni tekshiring yoki Railway da PostgreSQL qo'shing!")
        raise
